...
describe Linkage::Scenario do
  ...
  describe 'when self-joining' do
    ...
    describe "#run" do
      ...
      describe "when using non-optimized matches" do
        before(:each) do
          ...
          (1..4).each do |i|
            @cache.stub!(:fetch).with(i).and_return(@cached_records[i])
            @cache.stub!(:fetch).with(((i+1)..4).to_a).and_return(@cached_records[i+1, 4])  if i < 4
          end
          ...
          # matcher setup
          @matcher.stub!(:score).with(@cached_records[1], @cached_records[2..4]).and_return({
            'confident' => [[1, 3, 100]], 'unsure' => [[1, 4, 50]]
          })
          @matcher.stub!(:score).with(@cached_records[2], @cached_records[3..4]).and_return({
            'unsure' => [[2, 4, 50]]
          })
          @matcher.stub!(:score).with(@cached_records[3], @cached_records[4..4]).and_return({
            'unsure' => [[3, 4, 50]]
          })
        end
        ...
        it "should match all records" do
          @matcher.should_receive(:score).with(@cached_records[1], @cached_records[2..4]).and_return({
            'confident' => [[1, 3, 100]], 'unsure' => [[1, 2, 0]]
          })
          @matcher.should_receive(:score).with(@cached_records[2], @cached_records[3..4]).and_return({
            'unsure' => [[2, 4, 50]]
          })
          @matcher.should_receive(:score).with(@cached_records[3], @cached_records[4..4]).and_return({
            'unsure' => [[3, 4, 50]]
          })
          s = create_scenario
          s.run
